...
export async function onRequestGet({ request, env }) {
  ...
  try {
    const [products, inventory, orders] = await Promise.all([
      ...
      supabaseFetch(env, "/checkout_orders?select=id,order_number,status,customer_email,stripe_customer_email,shipping_method,subtotal_cents,shipping_cents,total_before_tax_cents,stripe_session_id,expires_at,paid_at,released_at,created_at,checkout_order_items(id,title,format,language,category,set_code,quantity,unit_amount_cents,line_amount_cents)&order=created_at.desc&limit=60")
    ]);
